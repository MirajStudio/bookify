...
export default async function ManagePage({
  ...
  if (!booking || booking.status !== "confirmed" || new Date(booking.endUtc) < new Date()) notFound();

  const labelDate = formatInTimeZone(new Date(booking.startUtc), booking.guestTimezone, "EEEE, MMMM d");
  const labelTime = formatInTimeZone(new Date(booking.startUtc), booking.guestTimezone, "h:mm a");
